...
int
lmt_cbr_print_client_metrics (void) {
    cerebro_t ch = NULL;
    List l = NULL;
    ListIterator itr;
    cmetric_t c;
    char *name = "lmt_client";
    char *s, *node;
    time_t trcv;
    char data_str[CEREBRO_MAX_DATA_STRING_LEN];
    int retval = -1;


    if (!(ch = cerebro_handle_create()))
        goto done;
    l = list_create ((ListDelF)_destroy_cmetric);
    if (_get_metric_data (ch, name, l) < 0)
        goto done;
    ...
    while ((c = list_next (itr))) {
        ...
        //sprintf(data_str, "%s;%ld;%s", node, trcv, s);
        sprintf(data_str, "%s;%s", node, s);
	printf("%s\n", data_str);
    }
